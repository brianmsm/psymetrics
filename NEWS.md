...
-   **Testing**: Expanded SEM coverage for path-only `lavaan::sem()` objects in `model_fit()`/`compare_model_fit()`, and added a manual stress script (`tests/manual/sem_stress_lavaan.R`) for broad SEM matrix checks.
...
-   **Plotting**: `plot_factor_loadings(verbose = FALSE)` now suppresses non-fatal messages/warnings from internal `lavaan` extraction calls, while still raising errors.
